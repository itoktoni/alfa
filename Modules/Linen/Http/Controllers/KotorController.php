...
class KotorController extends Controller
{
    ...
    public function data(KotorDataService $service)
    {
        return $service
            ...
            ->EditStatus([
                KotorFacades::mask_status() => TransactionStatus::class,
                KotorFacades::mask_description() => LinenStatus::class,
            ])
    ...
    public function show($code)
    {
        $model = $this->get($code);
        $detail = false;
        if($map = $model->has_detail){

            $detail = $map->mapToGroups(function($item){
