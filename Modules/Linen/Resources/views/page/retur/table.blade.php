    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="Product">
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->linen_retur_detail_rfid ?? '' }}"
                    name="detail[{{ $loop->index }}][temp_id]">

                <input type="text" readonly class="form-control"
                    value="{{ $item['temp_product'] ?? $item->linen_retur_detail_rfid ?? old('temp_id') }}"
                    name="detail[{{ $loop->index }}][temp_product]">

            </td>
            <td data-title="Description">
            <input type="text" readonly class="form-control"
                    value="{{ $item['temp_product'] ?? $item->linen_retur_detail_product_name ?? old('temp_id') }}"
                    name="detail[{{ $loop->index }}][temp_product]">
            </td>
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}1" name="detail[{{ $loop->index }}][temp_qty]"
                    ...
                    value="{{ $item['temp_qty'] ?? $item->linen_retur_detail_scan_company_name }}">
            </td>
            <td data-title="Send" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}2" name="detail[{{ $loop->index }}][temp_out]"
                    class="form-control" readonly
                    value="{{ $item['temp_qty'] ?? $item->linen_retur_detail_ori_location_name }}">

            </td>
            <!-- <td data-title="Send" class="text-right col-lg-1">
                
                <a class="btn btn-danger btn-block" href="{{ route('linen_retur_delete_detail', ['code' => $item->linen_retur_detail_id]) }}">Delete</a>
