...
class ReportBersihRepository extends GroupingDetail implements FromView, WithColumnFormatting, WithColumnWidths, GenerateReport
{
    ...
    public function data()
    {
        ...
        if ($from = request()->get('from')) {
            $kotor_from = request()->get('from') ?? null;
            $query = $query->where('linen_grouping_detail_reported_date', '>=', $kotor_from);
        }
        if ($to = request()->get('to')) {
            $kotor_to = request()->get('to') ?? null;
            $query = $query->where('linen_grouping_detail_reported_date', '<=', $kotor_to);
        }
